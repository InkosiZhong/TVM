...
class ImportancePrecisionTwoStageSelector(BaseSelector):
    ...
    def select(self, batch_size=1) -> Sequence:
        ...
        samp_labels = self.data.lookup(samp_ids, batch_size)
        ...
        cutoff_ub = min(cutoff_ub, n) # no more then n
        ...
        samp2_labels = self.data.lookup(samp2_ids, batch_size)
