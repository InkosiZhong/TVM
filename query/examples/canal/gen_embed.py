...
from tile import TileFullScanDataset, TileRandomSelectDataset, str2rect, intersect_rect
...
#TEST_DATA = os.path.join(ROOT_DATA_DIR, '2018-01-20-tile')
#TEST_DATA = os.path.join(ROOT_DATA_DIR, '2018-01-20-tile-all')
DEC_TYPE = VPFDecoder
...
embedding_dnn_transform_fn = \
    create_embedding_dnn_transform_fn(
        frame_shape=(1080, 1920),
        valid_area=valid_area,
        ban_area=ban_area
    )

class CanalTiledIndex(Index):
...
class CanalTiledConfig(IndexConfig):
    def __init__(self):
        super().__init__()
        self.cache_root = 'query/examples/canal/cache'
